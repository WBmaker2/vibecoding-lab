...
function isPrivateIpv6(address) {
    ...
    return (value === 0n ||
        ...
        first16 === 0xfec0n ||
        (value >> 121n) === 126n ||
        SPECIAL_IPV6_PREFIXES.some(([prefix, length]) => hasIpv6Prefix(value, prefix, length)));
}
...
function withDeadline(operation, deadline, signal) {
...
async function validateRemoteUrl(input, options) {
    ...
    const addresses = await withDeadline(lookupPromise, options.deadline, options.signal);
...
export async function assertSafeRemoteHttpUrl(input, options = {}) {
...
function toRequestHeaders(headers) {
    ...
    const values = headers
        ...
        : {};
    return {
        ...Object.fromEntries(Object.entries(values).filter(([name]) => !blockedHeaders.has(name))),
...
function requestPinnedResponse(validated, options) {
    ...
    return withDeadline(new Promise((resolve, reject) => {
        ...
        if (options.budget) {
            if (!Number.isSafeInteger(options.budget.remaining) ||
        ...
        request.end();
    }), options.deadline, options.signal);
}
function isRedirectStatus(status) {
    return [301, 302, 303, 307, 308].includes(status);
}
function exactContentType(headers) {
...
export async function fetchSafeHtml(input, options = {}) {
    const timeoutMs = options.timeoutMs ?? DEFAULT_TIMEOUT_MS;
    const maxBytes = options.maxBytes ?? DEFAULT_MAX_BYTES;
    const maxRedirects = options.maxRedirects ?? DEFAULT_MAX_REDIRECTS;
    const deadline = options.deadline ?? Date.now() + timeoutMs;
    const abort = combineAbortSignals(options.signal, Math.max(0, deadline - Date.now()));
    try {
